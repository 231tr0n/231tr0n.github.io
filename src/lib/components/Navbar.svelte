<script lang="ts">
	...
	const toggleTheme = () => {
		...
		darkMode().dark = isDark;
		localStorage.setItem(themeStorageKey, isDark ? themeDark : themeLight);
	};
...
<header class="zeltron-body-background">
	...
	<button
		...
		type="button">
		{#if darkMode().dark}
			<svg fill="currentColor" viewBox="0 0 45.74 45.74" xmlns="http://www.w3.org/2000/svg">
				<path
